...
def main():
    ...
    log_path = str(Path().absolute()) + str(Path("/logs/trainingdata_hoeffdingTreeClassifier.txt"))
    ...
    # preparing the labels for the training dataset
    ...
    # save the training data of the model as data stream
    ...
    # check if this the first time to train a model
    if not model: 
...
main()
log_path_exec = str(Path().absolute()) + str(Path("/logs/exec_training_hoeffdingTreeClassifier.txt"))
