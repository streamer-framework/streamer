...
class BaseTest:
    ...
    def run(self):
        ...
        try:
            ...
            serialized_model = self.redis_conn.get(redis_key_model)
            model = self.deserializeModel(serialized_model)
        ...
        #self.redis_conn.set(redis_key_output, predictions)
        #self.redis_conn.set(redis_key_output_sep, sep)
        ...
        if type(list_pred) is tuple:
            ...
            for i in range (len(means)):
                ...
                self.redis_conn.rpush(redis_key_output, str(value))   #push the outputs in redis (as a list)
        else:
            #print("I am not a tuple")
            for v in list_pred: #push the outputs in redis (as a list)
                self.redis_conn.rpush(redis_key_output, str(v)) 
    ...
    def deserializeModel(self, model):
