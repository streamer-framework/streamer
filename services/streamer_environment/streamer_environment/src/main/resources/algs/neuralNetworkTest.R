...
#target <- redisLRange(paste0("datatest",numTopic,"target"),0,-1) 
...
#redisRPush(paste("outputs",numTopic,sep = ""),charToRaw(resultStr) )

#we use rpush to store a list for a record prediction each element
redisRPush(paste("outputs",numTopic,sep = ""),charToRaw("NaN") )#first element is just headers, no prediction for it
for (i in 1:length(Alarmecl[[1]])) { #first element is just the headers
...
#predictionKey = paste("outputs",numTopic,sep = "")
#redisConnect()
